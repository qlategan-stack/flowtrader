...
with st.sidebar:
    ...
    st.caption("Toggle on/off to A/B test their signal impact. Changes take effect on the next bot cycle.")

    try:
        from strategies.engine import StrategyEngine
        _strat_engine = StrategyEngine()
        _strat_meta   = _strat_engine.get_strategy_metadata()
        _strat_config = _strat_engine.all_strategies()

        _role_labels = {"per_symbol": "per-symbol", "multi_asset": "multi-asset"}
        _dep_badge   = {"none": "", "scipy": " (needs scipy)", "pywt": " (needs pywt)", "ripser": " (needs ripser)"}

        _changed = False
        _new_config: dict = {}
        for key, meta in _strat_meta.items():
            label  = meta["label"]
            role   = _role_labels.get(meta["role"], meta["role"])
            deps   = _dep_badge.get(meta.get("deps", "none"), "")
            current_val = _strat_config.get(key, {}).get("enabled", False)
            new_val = st.toggle(
                f"{label}",
                value=current_val,
                key=f"strat_{key}",
            ...
            _new_config[key] = {"enabled": new_val}
            if new_val != current_val:
                _changed = True

        if _changed:
            try:
                StrategyEngine.save_config(_new_config)
                st.success("Strategy config saved.")
            except Exception as _e:
                st.error(f"Could not save: {_e}")

        _active = [k for k, v in _new_config.items() if v["enabled"]]
        if _active:
            st.caption(f"Active: {len(_active)} strategy/strategies")
        else:
            st.caption("All strategies disabled — pure TA mode")

    except ImportError:
        st.caption("strategies/ module not found")
...
with tab_market:
    ...
    # ── Math Signals panel ───────────────────────────────────────────────────
    any_math = any(
        item.get("indicators", {}).get("math_signals") or item.get("portfolio_math_signals")
        for item in wl
    )
    if any_math:
        st.divider()
        st.subheader("🔬 Mathematical Signal Output")
        st.caption("Signals generated by enabled math strategies — separate from the standard TA score.")

        # Portfolio-level signals (same on every item — just read from first)
        port_math = wl[0].get("portfolio_math_signals", {}) if wl else {}
        if port_math:
            with st.expander("Portfolio-Level Signals (multi-asset strategies)", expanded=True):
                if "transfer_entropy" in port_math:
                    te = port_math["transfer_entropy"]
                    if "error" not in te:
                        st.markdown("**Transfer Entropy Network**")
                        te_c1, te_c2 = st.columns(2)
                        te_c1.success("🔼 Leaders (info exporters): " + ", ".join(te.get("leaders", [])[:4]) or "—")
                        te_c2.info("🔽 Followers (info receivers): " + ", ".join(te.get("followers", [])[:4]) or "—")
                if "rmt" in port_math:
                    rmt = port_math["rmt"]
                    if "error" not in rmt:
                        st.markdown("**RMT Correlation**")
                        rmt_c1, rmt_c2, rmt_c3 = st.columns(3)
                        rmt_c1.metric("Significant Modes", rmt.get("significant_eigenvalues", "—"))
                        rmt_c2.metric("Market Mode Strength", f"{rmt.get('market_mode_strength', 0):.2f}×")
                        rmt_c3.metric("Noise Fraction", f"{rmt.get('noise_fraction', 0):.0%}")
                        mf = rmt.get("market_factor_coins", [])
                        if mf:
                            st.caption(f"Market factor driven by: {', '.join(mf)}")
                        if rmt.get("regime_shift_signal"):
                            st.warning("⚠️ Secondary regime shift forming (2nd eigenvalue approaching MP bound)")
                if "wasserstein" in port_math:
                    wass = port_math["wasserstein"]
                    if "error" not in wass:
                        st.markdown("**Wasserstein Regime**")
                        stab = wass.get("portfolio_stability", 0)
                        stab_icon = "🟢" if stab >= 0.75 else "🟡" if stab >= 0.5 else "🔴"
                        st.progress(stab, text=f"{stab_icon} Portfolio stability: {stab:.0%} of symbols in stable regime")
                        shifting = wass.get("shifting_symbols", [])
                        if shifting:
                            st.warning(f"Distribution shifting: {', '.join(shifting)}")
                if "tda" in port_math:
                    tda = port_math["tda"]
                    if "error" not in tda:
                        st.markdown("**TDA Persistent Homology (BTC)**")
                        if tda.get("crash_warning"):
                            st.error(f"⚠️ Topological crash warning — H1 loops: {tda.get('h1_count')} | entropy: {tda.get('h1_entropy'):.3f}")
                        else:
                            st.success(f"No topological stress — H1 loops: {tda.get('h1_count')} | entropy: {tda.get('h1_entropy', 0):.3f}")

        # Per-symbol math signals
        math_items = [item for item in wl if item.get("indicators", {}).get("math_signals")]
        if math_items:
            st.markdown("**Per-Symbol Signals**")
            math_rows = []
            for item in math_items:
                ms = item["indicators"]["math_signals"]
                row = {"Symbol": item["symbol"]}
                h = ms.get("hurst", {})
                if h and "error" not in h:
                    row["Hurst H"] = h.get("h", "—")
                    row["Hurst Signal"] = h.get("interpretation", "—")
                w = ms.get("wavelet", {})
                if w and "error" not in w:
                    row["Wavelet Regime"] = w.get("frequency_regime", "—")
                    row["Noise Ratio"] = w.get("noise_ratio", "—")
                e = ms.get("entropy", {})
                if e and "error" not in e:
                    row["Entropy Regime"] = e.get("regime", "—")
                lv = ms.get("levy", {})
                if lv and "error" not in lv:
                    row["Jump Regime"] = lv.get("regime", "—")
                    row["Jump Freq"] = lv.get("jump_frequency", "—")
                math_rows.append(row)
            if math_rows:
                st.dataframe(pd.DataFrame(math_rows), use_container_width=True, hide_index=True)
